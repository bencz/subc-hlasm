 ...
 *	Definitions
 *
 *	Modified to use cross-compiler target architecture.
 *	Target selection is now done at runtime via cg_set_target().
 */
...
#include <string.h>
#include <ctype.h>
#include "targets/arch/cgtarget.h"
#include "targets/arch/cgen_compat.h"

/*
 * Note: sys.h is no longer needed - OS, ASCMD, LDCMD, SYSLIBC, and AOUTNAME
 * are now defined as macros in cgen_compat.h that read from the active target.
 */
...
/*
 * AOUTNAME is now defined in cgen_compat.h from CG->aout_name
 * but we need a fallback before target is selected
 */
...
/*
 * Type sizes are now obtained from the active target.
 * BPW, INTSIZE, PTRSIZE are defined as macros in cgen_compat.h
 * that read from CG->bpw, CG->int_size, CG->ptr_size.
 */
#define CHARSIZE	1
