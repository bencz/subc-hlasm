 ...
 *	Main program
 *
 *	Modified to support cross-compiler target selection.
 */
...
static void version(void) {
	printf("SubC version %s for %s\n", VERSION, CG ? CG->name : "(no target)");
}
...
int main(int argc, char *argv[]) {
	...
	/* Select target (default to 386 if not specified) */
	if (!target) target = "386";
	if (!cg_set_target(target)) {
		fprintf(stderr, "scc: unknown target '%s'\n", target);
		fprintf(stderr, "Use -L to list available targets.\n");
		exit(EXIT_FAILURE);
	}
